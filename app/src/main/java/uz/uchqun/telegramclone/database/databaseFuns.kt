...
inline fun putUrlToDatabase(url: String, crossinline function: () -> Unit) {
    //sends the resulting URL to the database
...
inline fun getUrlFromStorage(path: StorageReference, crossinline function: (url: String) -> Unit) {
    //gets the URL of the image from the storage
    path.downloadUrl
...
inline fun putFileToStorage(uri: Uri, path: StorageReference, crossinline function: () -> Unit) {
    //sends the image to the storage
    path.putFile(uri)
        .addOnSuccessListener { function() }
        .addOnFailureListener { showToast(it.message.toString()) }
}
...
inline fun initUser(crossinline function: () -> Unit) {
    //initializing the current USER model
    REF_DATABASE_ROOT.child(NODE_USERS).child(CURRENT_UID)
...
fun updatePhonesToDatabase(arrayContacts: ArrayList<CommonModel>) {
    //The function adds a phone number with an id to the database.
...
//The function converts the received data from Firebase to the Common Model
...
private fun deleteOldUsername(newUserName: String) {
    ...
    REF_DATABASE_ROOT.child(NODE_USERNAME).child(
        USER.username
    ).removeValue()
        .addOnSuccessListener {
            showToast(
