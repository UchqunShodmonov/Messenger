package uz.uchqun.telegramclone.ui.screens.single_chats
...
import uz.uchqun.telegramclone.ui.message_recycler_view.views.AppViewFactory
...
class SingleChatFragment(private val contact: CommonModel) :
    ...
    private lateinit var mAppVoiceRecord: AppVoiceRecord
    ...
    private fun initFields() {
        ...
        mAppVoiceRecord = AppVoiceRecord()
        mSwipeRefreshLayout = swipe_refresh
        ...
        CoroutineScope(Dispatchers.IO).launch {
            chat_btn_voice.setOnTouchListener { v, event ->
                if (checkPermision(RECORD_AUDIO)) {
                    if (event.action == MotionEvent.ACTION_DOWN) {
                        ...
                        mAppVoiceRecord.startRecord(messageKey)
                    } else if (event.action == MotionEvent.ACTION_UP) {
                        //stop record
                        chat_input_message.setText("")
                        chat_btn_voice.colorFilter = null
                        mAppVoiceRecord.stopRecord { file, messageKey ->
                            uploadFileToStorage(
                                Uri.fromFile(file),
                                messageKey,
                                contact.id,
    ...
    private fun initRecyclerView() {
        ...
        single_chat_recycler_view.adapter = mAdapter
        single_chat_recycler_view.setHasFixedSize(true)
        single_chat_recycler_view.isNestedScrollingEnabled = false
        single_chat_recycler_view.layoutManager = mLayoutManager
        ...
        mMessagesListener = AppChildEventListener { snapshot ->
            ...
            if (mSmoothScrollToPosition) {
                mAdapter.addItemToBottom(AppViewFactory.getView(message)) {
                    single_chat_recycler_view.smoothScrollToPosition(mAdapter.itemCount)
                }
            } else {
        ...
        single_chat_recycler_view.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            ...
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (mIsScrolling && dx < 0 && mLayoutManager.findFirstVisibleItemPosition() <= 3) {
                    updateData()
                }
            }
        })
